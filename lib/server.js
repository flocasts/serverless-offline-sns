...
module.exports.create = (serverless, options) => {
    ...
    _connectionOptions = {
        host: _options.host || process.env.SNS_HOST || '0.0.0.0',
...
const runLambda = module.exports.runLambda = (params, lambda, name) => {
    ...
    lambda(snsEvent, lambdaContext, lambdaContext.done);
};
    ...
    addHandler((request, reply) => {
        if (typeof request.payload === 'object') {
            if (request.payload === null) {
                _log('Null payload recieved in request.');
            ...
            if (request.payload.TopicArn) {
                ...
                // Deconstruct the topicArn and find the topic.
                let arr = topicArn.split(':');
                const [a, b, c, region, aNumber, topicKey] = (arr.length === 6) ? arr: [null, null, null, null, null, arr.join(':')];

                const topic = _topics[topicKey];
                if (typeof topic !== 'undefined') {
                    ...
                    topic.handlers.forEach(handlerItem => {
                        ...
                        snsEvent.Records[0].EventSubscription = `${topicKey}:${handlerItem.subscriptionId}`;
